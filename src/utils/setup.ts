...
export class ProviderInstance {
  ...
  private static getRpcUrl(): string {
    ...
    if (network === 'testnet') {
      return process.env.FUEL_TESTNET_RPC || 'https://testnet.fuel.network/v1/graphql';
    } else if (network === 'ignition') {
      return process.env.FUEL_IGNITION_RPC || 'https://mainnet.fuel.network/v1/graphql';
    }
    
    throw new Error('Invalid FUEL_NETWORK value. Must be either "testnet" or "ignition"');
  }
  ...
  public static async getProvider(): Promise<Provider> {
    if (!ProviderInstance.instance) {
      ...
      console.log(`Connecting to Fuel network: ${process.env.FUEL_NETWORK} at ${rpcUrl}`);
